...
const ProductPlatformDetail: React.FC = () => {
  ...
  const courseOverviewApiCall = async () => {
    const res = await api.get(`${API_ENDPOINTS.productPlatform}/${id}`);
    if (res.status) {
  ...
  const handleBack = () => {
    navigate(`${base}platform-tutorial`);
  };
...
export default ProductPlatformDetail;